...
function StageBadge({
  ...
  if (status === "done") {
    return (
      <span className="grid size-10 place-items-center rounded-[var(--syn-radius)] bg-[var(--syn-accent)] text-[14px] font-semibold text-white">
        ✓
      </span>
    );
  }
  return (
    <span
      className={`grid size-10 place-items-center rounded-[var(--syn-radius)] border text-[14px] font-semibold ${
        status === "active"
...
function LessonCard({ lesson }: { lesson: RoadmapLesson }) {
  return (
    <article
      className={`min-h-[82px] rounded-[var(--syn-radius)] p-4 transition ${
        lesson.active
          ? "border-[1.5px] border-[var(--syn-accent)] bg-[var(--syn-accent-soft)]"
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        {lesson.done && (
          <span className="text-[14px] text-[var(--syn-accent)]">✓</span>
        )}
        {lesson.active && (
          <span className="rounded-[3px] bg-[var(--syn-accent)] px-2 py-0.5 text-[11px] font-semibold text-white">
            在做
          </span>
        )}
      </div>
...
function TaskBox({ status }: { status: RoadmapTaskStatus }) {
  if (status === "done") {
    return (
      <span className="grid size-4 place-items-center rounded-[3px] bg-[var(--syn-accent)] text-[10px] font-bold text-white">
        ✓
      </span>
    );
  }
